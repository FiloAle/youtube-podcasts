...
export function parseRelativeDate(text: string): number {
  ...
  if (lower.includes('minute') || lower.includes('minuto') || lower.includes('minuti') || lower.includes('min')) return num * 60;
  if (lower.includes('hour') || lower.includes('ora') || lower.includes('ore') || lower.includes('hr')) return num * 3600;
  if (lower.includes('day') || lower.includes('giorno') || lower.includes('giorni')) return num * 86400;
  if (lower.includes('week') || lower.includes('settimana') || lower.includes('settimane') || lower.includes('wk')) return num * 86400 * 7;
  if (lower.includes('month') || lower.includes('mese') || lower.includes('mesi') || lower.includes('mo')) return num * 86400 * 30;
  if (lower.includes('year') || lower.includes('anno') || lower.includes('anni') || lower.includes('yr')) return num * 86400 * 365;
  return Infinity;
}
...
class YouTubeService {
  ...
  async searchChannels(query: string): Promise<ChannelInfo[]> {
    ...
    return channels.map((ch: any) => {
      ...
      let thumbUrl = thumbnails.length > 0 ? thumbnails[thumbnails.length - 1].url : '';
  ...
  async getChannelInfo(id: string): Promise<ChannelInfo> {
    ...
    const metadataRows = (channel.header as any)?.content?.metadata?.metadata_rows ?? [];
    for (const row of metadataRows) {
      for (const part of (row.metadata_parts ?? [])) {
        const text = part.text?.text ?? (typeof part.text === 'string' ? part.text : '');
        if (text.toLowerCase().includes('subscriber') || text.toLowerCase().includes('iscritti')) {
          subscribers = text;
        }
      }
    }
  ...
  async getChannelVideos(channelId: string): Promise<VideoInfo[]> {
    ...
    const videos = allRawVideos.map((video: any) => {
      ...
      for (const badge of badges) {
        ...
        if (text.includes('member') || text.includes('abbonat') || text.includes('sponsor')) {
          isMembersOnly = true;
        }
      }
      if (isMembersOnly) return null;

      const title = video.metadata?.title?.text ?? video.title?.text ?? 'Senza titolo';
      ...
      for (const row of metadataRows) {
        for (const part of (row.metadata_parts ?? [])) {
          ...
          if (text.includes('view') || text.includes('visualizzazion')) {
            viewCount = text;
          } else if (text.includes('ago') || text.includes('fa') || text.includes('hour') || text.includes('day') ||
                     text.includes('week') || text.includes('month') || text.includes('year') || text.includes('Streamed') || text.includes('Trasmesso')) {
            publishedDate = text.replace(/Trasmesso in streaming /i, '').replace(/Streamed /i, '').trim();
          }
        }
      }

      const thumbImages = video.content_image?.image ?? [];
      const thumbnailUrl = Array.isArray(thumbImages) && thumbImages.length > 0
        ? thumbImages[0].url
        : `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;

      const channelName = channel.metadata?.title ?? '';

      return { title, videoId, duration, durationSeconds, viewCount, publishedDate, thumbnailUrl, channelName, channelId };
  ...
  async getAudioStream(videoId: string): Promise<AudioStreamInfo | null> {
    ...
    try {
      const info = await this.yt.getBasicInfo(videoId, { client: 'ANDROID_VR' });

      if (info.playability_status?.status === 'UNPLAYABLE') {
        throw new Error(info.playability_status.reason || 'Video non riproducibile (forse riservato agli abbonati)');
      }
      if (info.playability_status?.status === 'LIVE_STREAM_OFFLINE') {
        throw new Error(info.playability_status.reason || 'Questo evento dal vivo non è ancora iniziato.');
      }

      const progressive = (info.streaming_data?.formats ?? [])
        .filter((f: any) => f.has_audio && f.url);
      const fallbackUrl = progressive.length > 0 ? progressive[0].url : undefined;
      ...
      if (audioFormats.length > 0) {
        ...
        const bestM4a = audioFormats.find((f: any) => f.mime_type?.includes('mp4a'));
